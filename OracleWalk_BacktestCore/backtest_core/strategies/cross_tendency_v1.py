"""CrossTendency V1 — full MT5 EA reproduction with trailing start threshold.

Identical to CrossTendency but with proper trailing activation: the trailing
stop only engages after the trade reaches a minimum profit percentage (default
1.0%). Until then, the stop stays fixed at the daily pivot level.
"""
...
@register_strategy("cross_tendency_v1", aliases=("crossTendency_v1",))
class CrossTendencyV1Strategy(StrategyBase):
    name = "CrossTendency V1"
    ...
    def generate_signals(self, data: pd.DataFrame) -> pd.DataFrame:
        ...
        for i in range(1, n):
            ...
            stop_price[i] = stop
            comment[i] = f"CrossTendency V1 {'BUY' if direction == 1 else 'SELL'}"

            # --- Trailing with start threshold ---
            if self.use_trailing:
                trailing_distance[i] = close[i] * self.trailing_stop_pct
                trailing_start_profit[i] = self.trailing_start_pct
        ...
        if self.use_trailing:
            ...
            cols["trailing_start_profit"] = trailing_start_profit
